...
当前版本为 v1.1.3，在 v1.1.2 基础上修复了 MSVC 构建缺少 `/utf-8` 编译选项导致界面中文乱码的问题；v1.1.x 在 v1.0.0 首个正式版本基础上增强了快捷键操作（数字键切换标注工具、贴图窗口 `Ctrl+C` / `Ctrl+S` / `Esc`、取色器 `C` 键复制、截图过程中 `R` / `H` 进入取色器）并修复了若干问题，并修复了便携版 Windows manifest 兼容性声明导致的启动报错；核心截图、标注、设置、贴图、取色、录屏、文档、构建脚本和 GitHub Release 工作流均已完成；高 DPI、多显示器不同 DPI 和旧版 Windows 兼容性仍建议在真实设备上持续验证。
